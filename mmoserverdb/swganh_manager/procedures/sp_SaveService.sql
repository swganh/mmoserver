...
DELIMITER //
CREATE PROCEDURE `sp_SaveService`(IN `in_address` VARCHAR(255), IN `in_tcp_port` INT, IN `in_udp_port` INT, IN `in_ping_port` INT, IN `in_status` INT, IN `in_pulse` VARCHAR(50), IN `in_id` INT) SQL SECURITY INVOKER
BEGIN
