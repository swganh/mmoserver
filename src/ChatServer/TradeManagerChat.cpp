...
void TradeManagerChatHandler::Shutdown()
{
	...
	int8 sql[256];
	sprintf(sql,"UPDATE galaxy SET Global_Tick_Count = '%I64u' WHERE galaxy_id = '2'",getGlobalTickCount());
...
void TradeManagerChatHandler::handleDatabaseJobComplete(void* ref,DatabaseResult* result)
{
		...
		case TRMQuery_RetrieveAuction:
			{
				...
				if (TerminalRegionbyID(asynContainer->BazaarID) == TerminalRegionbyID(AuctionTemp.BazaarID)){
					...
					int8 sql[100];
					sprintf(sql,"DELETE FROM commerce_auction WHERE auction_id = '%I64u' ",AuctionTemp.ItemID);
		...
		case TRMQuery_GetDetails:
			{
				...
				if(strstr(mItemDescription->tang,"resource_container"))
				{
					...
					sprintf(sql,"SELECT rc.resource_id, rc.amount, r.name, r.er, r.cr,r.cd,r.dr,r.fl,r.hr,r.ma, r.oq, r.sr, r.ut, r.pe FROM swganh.resource_containers rc INNER JOIN swganh.resources r ON r.id = rc.resource_id WHERE rc.id =%I64u",mItemDescription->ItemID);
				...
				sprintf(sql,"SELECT name, value FROM swganh.item_attributes ia  INNER JOIN swganh.attributes a ON ia.attribute_id = a.id WHERE ia.item_id =%I64u and a.internal = 0 ORDER BY ia.order",mItemDescription->ItemID);
		...
		case TRMQuery_GetResAttributeDetails:
			{
				...
				if (count == 1)	
				{
					...
					sprintf(resItemDescription->value,"%u/100000",mItemDescription->amount);
					mAtrributesList.push_back(resItemDescription);
		...
		case TRMQuery_CancelAuction_BidderMail:
			{
				...
				int8 sql[100];
				sprintf(sql,"SELECT sf_CancelLiveAuction ('%I64u')",asynContainer->AuctionID);
		...
		case TRMQuery_CancelAuction:
			{
				...
				if (count == 1){
					...
					gLogger->logMsgF("TradeManager::TRMQuery_CancelAuction::Aucction not found : %I64u\n",MSG_NORMAL,asynContainer->AuctionID);
		...
		case TRMQuery_ExpiredListing:
			{
				...
				while(it != mAuction.end())
				{
					...
					{
						gLogger->logMsgF("check refund\n",MSG_NORMAL);
						//read in the bidhistory and iterate through the bidders of the auction
						//refund money to everybody but the winning bidder
						
						TradeManagerAsyncContainer* asyncContainer;
						sprintf(sql,"SELECT cbh.proxy_bid, c.id  FROM swganh.characters AS c INNER JOIN swganh.commerce_bidhistory AS cbh ON (c.firstname = cbh.bidder_name) WHERE cbh.auction_id = %I64u",auctionTemp->ItemID);
					...
					if((auctionTemp->AuctionTyp == TRMVendor_Auction) && (auctionTemp->BidderID != 0))
					{
						...
						sprintf(sql,"UPDATE swganh.commerce_auction SET owner_id = %I64u, bidder_name = '' WHERE auction_id = %I64u ",auctionTemp->BidderID,auctionTemp->ItemID);
			...
			case TRMQuery_ProcessAuctionRefund:
			{
				...
				for(uint64 i = 0;i < count;i++)
				{
					...
					if(AuctionTemp->BidderID != asynContainer->AuctionTemp->OwnerID){
						...
						sprintf(sql,"UPDATE banks SET credits=credits+%i WHERE id=%lld",asynContainer->MyProxy, AuctionTemp->BidderID+4);
						asyncContainer = new TradeManagerAsyncContainer(TRMQuery_NULL,NULL);
						mDatabase->ExecuteSqlAsync(this,asyncContainer,sql);	
					}
				...
				sprintf(sql,"DELETE FROM commerce_bidhistory WHERE auction_id = '%I64u' ",asynContainer->AuctionTemp->ItemID);
				asyncContainer = new TradeManagerAsyncContainer(TRMQuery_NULL,NULL);
				mDatabase->ExecuteSqlAsync(this,asyncContainer,sql);	
			...
			case TRMQuery_BidAuction:
			{
				...
				if (AuctionTemp->AuctionTyp == 0){
					...
					gLogger->logMsgF("TMR query bidauction\n",MSG_NORMAL);
					...
					*sqlPointer++ = '\0';
					...
					int8 sql[390];
					sprintf(sql,"SELECT cbh.proxy_bid, cbh.max_bid, c.id FROM swganh.characters AS c INNER JOIN swganh.commerce_bidhistory AS cbh ON c.firstname = '%s' WHERE cbh.auction_id = %I64u AND cbh.bidder_name = '",name,AuctionTemp->ItemID);
	...
	{
		mBazaarsLoaded = false;
		gLogger->logMsgF("TradeManager::Load complete (%u bazaars )\n",MSG_NORMAL,mBazaarCount);
	}
...
{
	gLogger->logMsgF("processAuctionEMails\n",MSG_NORMAL);
	...
	if(auctionTemp->AuctionTyp == TRMVendor_Instant)
	{
		...
		gLogger->logMsgF("trade manager vendor instant\n",MSG_NORMAL);
		...
		gLogger->logMsgF("trade manager vendor instant ended\n",MSG_NORMAL);

	}

	if(auctionTemp->AuctionTyp == TRMVendor_Auction)
	{
		gLogger->logMsgF("trm vendor auction\n",MSG_NORMAL);
		//has it been sold??
	...
	{
		gLogger->logMsgF("trm vendor auction ended\n",MSG_NORMAL);
...
{
//auction ... 
	gLogger->logMsgF("process auction bid\n",MSG_NORMAL);
	...
	*sqlPointer++ = '\0';				
	...
	if (asynContainer->AuctionTemp->BidderID == player->getCharId())
	{
		...
		if (asynContainer->MyProxy > asynContainer->AuctionTemp->HighProxy)
		{
			...
			//sprintf(sql," UPDATE commerce_bidhistory SET proxy_bid = '%u'WHERE auction_id = '%I64u' AND bidder_name = %s",asynContainer->MyProxy  ,asynContainer->AuctionTemp->ItemID ,Name);			
			sprintf(sql," UPDATE commerce_bidhistory SET proxy_bid = '%u'WHERE auction_id = '%I64u' AND bidder_name = '%s'",asynContainer->MyProxy  ,asynContainer->AuctionTemp->ItemID,PlayerName);			
	...
	else
	{
		...
		sprintf(sql,"SELECT sf_BidUpdate ('%I64u','%u','%u','%s')",asynContainer->AuctionTemp->ItemID,asynContainer->MyBid,asynContainer->MyProxy,PlayerName);
		TradeManagerAsyncContainer* asyncContainer; 
		asyncContainer = new TradeManagerAsyncContainer(TRMQuery_ACKRetrieval,asynContainer->mClient);
		mDatabase->ExecuteSqlAsync(this,asyncContainer,sql);
	...
	sprintf(sql,"SELECT sf_BidAuction ('%I64u','%u','%u','%s')",asynContainer->AuctionTemp->ItemID,TheBid,TheProxy,PlayerName);
	TradeManagerAsyncContainer* asyncContainer; 
	asyncContainer = new TradeManagerAsyncContainer(TRMQuery_ACKRetrieval,asynContainer->mClient);
...
void TradeManagerChatHandler::ProcessCreateAuction(Message* message,DispatchClient* client)
{
	...
	if (category != 0)
	{
		sprintf(Query," UPDATE commerce_auction SET start = '%I64u', region_id = '%u', planet_id = '%u',category = '%u' WHERE auction_id = '%I64u'", time, RegionID, player->getPlanetId(),category,ItemID);			
	}
	else
	{
		sprintf(Query," UPDATE commerce_auction ca inner join swganh.items i ON (i.id = ca.auction_id) inner join swganh.item_types it ON (it.id = i.item_type) SET ca.start = '%I64u' , ca.region_id = '%u', ca.planet_id = '%u', ca.category = it.bazaar_category  WHERE ca.auction_id = '%I64u'", time, RegionID, player->getPlanetId(),ItemID);			
	}
...
void TradeManagerChatHandler::processRetrieveAuctionItemMessage(Message* message,DispatchClient* client)
{
	...
	int8 sql[200];
	sprintf(sql,"SELECT auction_id, bazaar_id, itemtype FROM swganh.commerce_auction c  WHERE c.auction_id = %I64u",ItemID);
...
void TradeManagerChatHandler::processBidAuctionMessage(Message* message,DispatchClient* client)
{
	...
	{
		gLogger->logMsgF("processBidAuctionMessage :: Error getting the player from account map %u\n",MSG_NORMAL,client->getAccountId());
		return;
	}
	...
	sprintf(sql,"SELECT ca.auction_id, ca.owner_id, ca.type, ca.category, ca.price, ca.itemtype, ca.name, ca.bazaar_id, ca.bidder_name, c.firstname FROM swganh.characters AS c INNER JOIN swganh.commerce_auction AS ca ON (ca.owner_id = c.id) where ca.auction_id = %I64u",ItemID);
...
void TradeManagerChatHandler::processCancelLiveAuctionMessage(Message* message,DispatchClient* client)
{
	...
	//send the EMails
	int8 sql[300];
	sprintf(sql,"SELECT ch.id, ca.name, cbh.proxy_bid FROM swganh.commerce_auction AS ca INNER JOIN swganh.commerce_bidhistory AS cbh ON (cbh.bidder_name = ca.bidder_name)  INNER JOIN swganh.characters AS ch ON (cbh.bidder_name = ch.firstname)  WHERE ca.auction_id = %I64u",ItemID);
...
void TradeManagerChatHandler::processGetAuctionDetails(Message* message,DispatchClient* client)
{
	...
	sprintf(sql,"SELECT auction_id, description, object_string FROM swganh.commerce_auction c  WHERE c.auction_id = %I64u",AuctionID);
...
void TradeManagerChatHandler::processHandleopAuctionQueryHeadersMessage(Message* message,DispatchClient* client)
{
	...
	{
		gLogger->logMsgF("Error getting player from account map \n",MSG_NORMAL);
		return;
	}
	//squeeze our Packet for all usefull information
	Query query;
	...
	case TRMVendor:
		{
			sprintf(query.RegionQuery," (c.bazaar_id = %I64u)",query.vendorID);
	...
	case TRMRegion:
		{
			//for now Planet
			sprintf(query.RegionQuery," (c.region_id = %u)",TerminalRegionbyID(query.vendorID));
		}
		break;

	case TRMPlanet:
		{
			sprintf(query.RegionQuery," (c.planet_id = %u)",player->getPlanetId());
	...
	switch (query.Windowtype)
	{
		...
		{	
			sprintf(query.WindowQuery," ((c.type = %u) or (c.type = %u))",TRMVendor_Auction,TRMVendor_Instant);
		...
		case TRMVendor_MySales://what Im selling at the bazaar
		{
			...
			int8 tmp[128];
			sprintf(tmp," (c.owner_id = %I64u)",player->getCharId());
		...
		{
			sprintf(query.WindowQuery," (c.type = %u) AND (c.owner_id = %I64u) ",TRMVendor_Ended,player->getCharId());
		...
		{
			sprintf(query.WindowQuery," (c.type = %u) AND (c.bidder_name = %s) AND (c.bazaar_id = %I64u)",TRMVendor_Offer,player->getName().getAnsi(),query.vendorID);
		...
		{
			gLogger->logMsgF("trm vendor for sale\n",MSG_NORMAL);
			sprintf(query.WindowQuery," ((c.type = %u) or (c.type = %u)) AND",TRMVendor_Auction,TRMVendor_Instant);
			int8 tmp[128],end[128],*sqlPointer;
			...
			*sqlPointer++ = '\0';
			...
			sprintf(tmp," (c.bazaar_id = %I64u)",query.vendorID);
			strcat(query.WindowQuery,tmp);
	...
	if (category != 0)
	{
		if ((category << 24) == 0) 
		{
			sprintf(query.CategoryQuery," ((c.category >> 8) = %u)",(query.Category >> 8));

		}
		else
		{
			//were looking for a subcategory
			sprintf(query.CategoryQuery," ((c.category) = %u)",query.Category );
		}
	}
	...
	if (query.ItemTyp != 0){
		sprintf(query.ItemTypQuery," (c.itemtype = %u)",query.ItemTyp);
...
void TradeManagerChatHandler::processHandleIsVendorMessage(Message* message,DispatchClient* client)
{
	...
	if (mBazaarString == ""){
		error = 1;
		gLogger->logMsgF("TradeManager::_processIsVendorRequest: Planet  %u : Vendor %I64u not found in db\n",MSG_HIGH,player->getPlanetId(),mVendorId);
	}
...
void TradeManagerChatHandler::processTimerEvents()
{
	...
	while(mTimerEventQueue.size() && processTime < mTimerQueueProcessTimeLimit)
	{
		...
		switch(id)
		{
			...
			default:
				gLogger->logMsgF("WorldManager::processTimerEvents: Unknown Timer %u\n",MSG_HIGH,id);
			break;
		}
