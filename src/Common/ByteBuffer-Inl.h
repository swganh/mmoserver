...
template<typename T> void ByteBuffer::SwapEndian_(T& data) { 
...
template<typename T> const T ByteBuffer::Peek(bool do_swap_endian) {
	return PeekAt<T>(read_position_, do_swap_endian);
}

template<typename T> const T ByteBuffer::PeekAt(size_t offset, bool do_swap_endian) {
