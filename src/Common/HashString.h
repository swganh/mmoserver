...
public:
     ...
     */
    COMMON_API explicit HashString(char const * const ident_string);

    /// The default destructor.
    COMMON_API ~HashString();

    // @note: The default copy constructor and assignment operator functions
     ...
     */
    COMMON_API bool operator<(const HashString& other) const;
     ...
     */
    COMMON_API uint32_t ident() const;
    
    /**
     * \returns A unique identifier for this HashString.
     */
    COMMON_API const std::string& ident_string() const;
    
private:
    /// Disable the default constructor. Must use the explicit constructor.
    HashString();

    // @note: ident_ is stored as a void * not an int, so that in
