	...
	switch(asyncContainer->mQueryType)
	{
		...
		case DPFQuery_ObjectCount:
		{
			...
			if(objectCount != 0)
			{
				...
				mDatabase->ExecuteSqlAsync(this,asContainer,
						...
						" UNION (SELECT \'VehicleControllers\',VehicleControllers.id FROM VehicleControllers WHERE (parent=%"PRIu64"))"
						,dtpId-3,dtpId,dtpId);
		...
		case DPFQuery_Objects:
		{
			...
			for(uint32 i = 0;i < count;i++)
			{
				...
				else if(strcmp(queryContainer.mString.getAnsi(),"VehicleControllers") == 0)
				{
