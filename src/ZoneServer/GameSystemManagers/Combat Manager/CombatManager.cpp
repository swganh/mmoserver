...
CombatManager::CombatManager(swganh::database::Database* database) :
    mDatabase(database)
{
...
CombatManager* CombatManager::Init(swganh::database::Database* database)
{
    if(mInsFlag == false)
    {
...
void CombatManager::handleDatabaseJobComplete(void* ref,swganh::database::DatabaseResult* result)
{
    CMWeaponGroup*	weaponGroup;
    swganh::database::DataBinding*	binding = mDatabase->createDataBinding(3);

    binding->addField(swganh::database::DFT_uint32,offsetof(CMWeaponGroup,mId),4,0);
    binding->addField(swganh::database::DFT_uint32,offsetof(CMWeaponGroup,mDefaultAttackAnimationCrc),4,1);
    binding->addField(swganh::database::DFT_bstring,offsetof(CMWeaponGroup,mDefaultCombatSpam),64,2);
