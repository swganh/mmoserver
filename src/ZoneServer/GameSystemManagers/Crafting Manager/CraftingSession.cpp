...
//
CraftingSession::CraftingSession(Anh_Utils::Clock* clock,swganh::database::Database* database,PlayerObject* player,CraftingTool* tool,CraftingStation* station,uint32 expFlag)
    : mClock(clock)
    , mDatabase(database)
...
void CraftingSession::handleDatabaseJobComplete(void* ref,swganh::database::DatabaseResult* result)
{
    ...
    case CraftSessionQuery_CustomizationData:
    {
        ...
        binding->addField(swganh::database::DFT_bstring,offsetof(CustomizationOption,attribute),32,0);
        binding->addField(swganh::database::DFT_uint16,offsetof(CustomizationOption,cutomizationIndex),2,1);
        binding->addField(swganh::database::DFT_uint32,offsetof(CustomizationOption,paletteSize),4,2);
        binding->addField(swganh::database::DFT_uint32,offsetof(CustomizationOption,defaultValue),4,3);
...
void CraftingSession::createPrototype(uint32 noPractice,uint32 counter)
{
    if(noPractice)
    {
        ...
        swganh::database::Transaction* t = mDatabase->startTransaction(this,new CraftSessionQueryContainer(CraftSessionQuery_Prototype,static_cast<uint8>(counter)));
