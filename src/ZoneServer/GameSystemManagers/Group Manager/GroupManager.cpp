...
GroupManager::GroupManager(swganh::database::Database* database, MessageDispatch* dispatch)
{
...
GroupManager*	GroupManager::Init(swganh::database::Database* database, MessageDispatch* dispatch)
{
    if(!mInsFlag)
    {
...
void GroupManager::_processIsmGroupLootModeResponse(Message* message, DispatchClient* client)
{
    ...
    StringVector availableLootModes;
...
void GroupManager::_processIsmGroupInviteInRangeRequest(Message *message, DispatchClient* client)
{
	...
	if (glm::distance(sender->GetCreature()->getWorldPosition(), target->GetCreature()->getWorldPosition()) < 90)
    {
