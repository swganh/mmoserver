...
int32 FactoryCrate::decreaseContent(uint32 amount)
{
    ...
    gWorldManager->getKernel()->GetDatabase()->executeSqlAsync(0,0,"UPDATE %s.item_attributes SET value='%i' WHERE item_id=%"PRIu64" AND attribute_id=%u",gWorldManager->getKernel()->GetDatabase()->galaxy(),newAmount,this->getId(),AttrType_factory_count);
