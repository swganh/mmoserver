...
TradeManager::TradeManager(swganh::database::Database* database, MessageDispatch* dispatch)
: mDatabase(database)
...
//======================================================================================================================
TradeManager*	TradeManager::Init(swganh::database::Database* database, MessageDispatch* dispatch)
{
    if(!mInsFlag)
    {
...
//=======================================================================================================================
void TradeManager::handleDatabaseJobComplete(void* ref,swganh::database::DatabaseResult* result)
{
    ...
    case TRMQuery_ItemTableFrogQuery:
    {
        ...
        binding->addField(swganh::database::DFT_uint64,offsetof(ItemFrogItemClass,id),8,0);
        binding->addField(swganh::database::DFT_uint32,offsetof(ItemFrogItemClass,type),4,1);
        binding->addField(swganh::database::DFT_uint32,offsetof(ItemFrogItemClass,family),4,2);
        binding->addField(swganh::database::DFT_bstring,offsetof(ItemFrogItemClass,name),32,3);
        binding->addField(swganh::database::DFT_uint8,offsetof(ItemFrogItemClass,active),1,4);
        binding->addField(swganh::database::DFT_bstring,offsetof(ItemFrogItemClass,f_name),32,6);
...
void TradeManager::_processAddItemMessage(Message* message,DispatchClient* client)
{
    ...
    if(!(playerObject && playerObject->isConnected()))
    {
		LOG(error) << "TradeManager::_processAddItemMessage playeracount " << client->getAccountId() <<" : no player";
        return;
    }

    if (!playerObject->getTradeStatus())
    {
		LOG(error) << "TradeManager::_processAddItemMessage player " << playerObject->getId() << "not trading!!!!!!!!";
        return;
    }
    ...
    if (!addedItem)
    {
		LOG(error) << "TradeManager::_processAddItemMessage item : " << ItemId << "did not exist for player " << playerObject->getId();
        return;
    }
