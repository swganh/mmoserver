...
void ObjectController::_handleStructurePlacement(uint64 targetId,Message* message,ObjectControllerCmdProperties* cmdProperties)
{
	...
	if(player->checkState(CreatureState_Swimming)){
		...
		gMessageLib->SendSystemMessage(L"You cannot place a structure while swimming.", player);
		return;
	}
	if(player->checkState(CreatureState_MountedCreature)|| player->checkState(CreatureState_RidingMount)){
        gMessageLib->SendSystemMessage(::common::OutOfBand("player_structure", "cant_place_mounted"), player);
		return;
	}
	...
	if(!gStructureManager->checkCityRadius(player))//TODO: Update to include ALL no-build areas, not just cities (pois, etc..)
	{
        ...
        gMessageLib->SendSystemMessage(::common::OutOfBand("player_structure", "not_permitted"), player);
		return;
	}
