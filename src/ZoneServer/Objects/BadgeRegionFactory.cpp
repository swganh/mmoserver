...
BadgeRegionFactory::BadgeRegionFactory(swganh::database::Database* database) : FactoryBase(database)
{}
...
void BadgeRegionFactory::handleDatabaseJobComplete(void* ref,swganh::database::DatabaseResult* result)
{
}
//=============================================================================
...
void BadgeRegionFactory::requestObject(ObjectFactoryCallback* ofCallback,uint64 id, uint16 subGroup, uint16 subType, DispatchClient* client)
{
    ...
    mDatabase->executeAsyncProcedure(sql, [=](swganh::database::DatabaseResult* result) {
        if(!result) {
            return;
        }
