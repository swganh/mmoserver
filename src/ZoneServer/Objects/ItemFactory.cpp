...
ItemFactory*	ItemFactory::Init(swganh::app::SwganhKernel*	kernel)
{
    if(!mInsFlag)
    {
        mSingleton = new ItemFactory(kernel);
        mInsFlag = true;
        return mSingleton;
    }
    else
...
ItemFactory::ItemFactory(swganh::app::SwganhKernel*	kernel) : FactoryBase(kernel)
{
    _setupDatabindings();
}
...
void ItemFactory::requestObject(ObjectFactoryCallback* ofCallback,uint64 id,uint16 subGroup,uint16 subType,DispatchClient* client)
{
	...
	sql << "SELECT items.id,items.parent_id,items.item_family,items.item_type,items.privateowner_id,items.oX,items.oY, items.oZ,items.oW,"
		<< "items.x,items.y,items.z,items.planet_id,items.customName,item_types.object_string,item_types.stf_name,item_types.stf_file,"
		<< "item_types.stf_detail_name,item_types.stf_detail_file,items.maxCondition,items.damage,items.dynamicint32,"
		<< "item_types.equipSlots,item_types.equipRestrictions, item_customization.1, item_customization.2, item_types.container "
		<< "FROM " << mDatabase->galaxy() << ".items INNER JOIN " << mDatabase->galaxy() << ".item_types ON (items.item_type = item_types.id) "
		<< "LEFT JOIN " << mDatabase->galaxy() << ".item_customization ON (items.id = item_customization.id) WHERE items.id = " << id << ";";
...
Item* ItemFactory::_createItem(swganh::database::DatabaseResult* result)
{
    Item*			item;
...
void ItemFactory::_setupDatabindings()
{
    ...
    mItemBinding->addField(swganh::database::DFT_float,offsetof(Item,mDirection.w),4,8);
    ...
    mItemBinding->addField(swganh::database::DFT_uint32,offsetof(Item,mMaxCondition),4,19);
    mItemBinding->addField(swganh::database::DFT_uint32,offsetof(Item,mDamage),4,20);
    mItemBinding->addField(swganh::database::DFT_uint32,offsetof(Item,mDynamicInt32),4,21);
    mItemBinding->addField(swganh::database::DFT_uint64,offsetof(Item,mEquipSlots),4,22);
    mItemBinding->addField(swganh::database::DFT_uint32,offsetof(Item,mEquipRestrictions),4,23);
    mItemBinding->addField(swganh::database::DFT_uint16,offsetof(Item,mCustomization[1]),2,24);
    mItemBinding->addField(swganh::database::DFT_uint16,offsetof(Item,mCustomization[2]),2,25);
    mItemBinding->addField(swganh::database::DFT_uint16,offsetof(Item,mCapacity),2,26);



    mItemIdentifierBinding = mDatabase->createDataBinding(2);
    mItemIdentifierBinding->addField(swganh::database::DFT_uint32,offsetof(ItemIdentifier,mFamilyId),4,2);
    mItemIdentifierBinding->addField(swganh::database::DFT_uint32,offsetof(ItemIdentifier,mTypeId),4,3);
}
