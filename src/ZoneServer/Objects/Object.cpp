...
bool Object::addObject(Object* data)
{
    ...
    {
		//assert(0);
		DLOG(info) << "Object* : "<< data->getId() <<"::addObject No Capacity left for container " << this->getId();
        return false;
    }
}
...
uint16 Object::getHeadCount() {
    ...
    std::for_each(mData.begin(), mData.end(), [&count] (uint64_t object_id) {
		...
		if(!object)	{
			LOG(error) << "Object::getHeadCount() Object not in the worldmanagers main Object Map!!!!!!!!! : " << object_id;
			return;
		}
